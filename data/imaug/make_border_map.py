...
class MakeBorderMap(object):
    ...
    def __call__(self, data):
        ...
        canvas = np.zeros(img.shape[:2], dtype=np.float32)  # gt
        ...
        # 计算后的值在0-1之间，需要rescale到0.3-0.7之间
    ...
    def draw_border_map(self, polygon, canvas, mask):
        ...
        polygon_shape = Polygon(polygon)  # 创建Polygon对象
        ...
        padded_polygon = np.array(padding.Execute(distance)[0])   # 正的distance是膨胀操作
        cv2.fillPoly(mask, [padded_polygon.astype(np.int32)], 1.0)  # 膨胀后的mask填充1
        ...
        # 两者结合，用于获取padded_polygon点的坐标
        xs = np.broadcast_to(np.linspace(0, width - 1, num=width).reshape(1, width), (height, width))
        ys = np.broadcast_to(np.linspace(0, height - 1, num=height).reshape(height, 1), (height, width))
        # 多边形有多少个点即有多少条边，用于存储计算padded_polygon上每个点到polygon的每条边的距离
        distance_map = np.zeros((polygon.shape[0], height, width), dtype=np.float32)
        for i in range(polygon.shape[0]):
            j = (i + 1) % polygon.shape[0]  # i、j组成边
            # 计算区域的每个点到ij组成边的距离
            absolute_distance = self._distance(xs, ys, polygon[i], polygon[j])
            # polygon内外距离distance内的值为0-1，更里面大于1，需要裁剪
